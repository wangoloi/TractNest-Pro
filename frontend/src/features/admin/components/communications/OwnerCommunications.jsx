...
const OwnerCommunications = () => {
  ...
  // Load real admins and messages from localStorage
  useEffect(() => {
    loadAdminsAndMessages();
  }, []);

  const loadAdminsAndMessages = () => {
    try {
      setLoading(true);
      ...
      // Transform admin users to match expected format
      const transformedAdmins = adminUsers.map((admin, index) => ({
        id: admin.username,
        name: admin.name,
        email: admin.email,
      ...
      const storedMessages = JSON.parse(localStorage.getItem('tracknest_messages') || '[]');
      
      // Filter messages to only show conversations between owner and admins
      const adminConversations = storedMessages.filter(message => {
        // Include messages where owner is sender or recipient
        const isOwnerInvolved = message.from === 'bachawa' || message.to === 'bachawa';
        
        // Include messages where the other party is an admin
        const otherParty = message.from === 'bachawa' ? message.to : message.from;
        const isAdminInvolved = adminUsers.some(admin => admin.username === otherParty);
        
        return isOwnerInvolved && isAdminInvolved;
      });
      
      // Transform messages to match expected format
      const transformedMessages = adminConversations.map((msg, index) => {
        const isOutgoing = msg.from === 'bachawa';
        const otherParty = isOutgoing ? msg.to : msg.from;
        const adminUser = adminUsers.find(admin => admin.username === otherParty);
        
        return {
          id: msg.id || `msg_${index}`,
          senderId: isOutgoing ? 'bachawa' : otherParty,
          senderName: isOutgoing ? 'Owner' : (adminUser?.name || otherParty),
          senderEmail: isOutgoing ? 'owner@tracknest.com' : (adminUser?.email || ''),
          recipientId: isOutgoing ? otherParty : 'bachawa',
          recipientName: isOutgoing ? (adminUser?.name || otherParty) : 'Owner',
          recipientEmail: isOutgoing ? (adminUser?.email || '') : 'owner@tracknest.com',
      ...
      console.log('✅ Loaded admin conversations:', transformedMessages);
  ...
  const handleSendMessage = () => {
    ...
    const recipient = admins.find(admin => admin.id === newMessage.recipientId);
    if (!recipient) {
      toast.error('Recipient not found');
      return;
    }

    // Create message in the format expected by localStorage
    const message = {
      id: `msg_${Date.now()}`,
      from: 'bachawa', // Owner username
      to: newMessage.recipientId, // Admin username
    ...
    try {
      ...
      const transformedMessage = {
        ...
        senderId: 'bachawa',
        senderName: 'Owner',
        senderEmail: 'owner@tracknest.com',
      ...
      setMessages([transformedMessage, ...messages]);
      setNewMessage({
        recipientId: '',
        subject: '',
        content: '',
  ...
  const replyToMessage = (message) => {
    setNewMessage({
      recipientId: message.senderId === 'bachawa' ? message.recipientId : message.senderId,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Admin Communications</h1>
              <p className="text-gray-600">Manage messages and communications with admin users only</p>
            </div>
            <div className="flex space-x-3">
              <button
                onClick={loadAdminsAndMessages}
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Only admin users are available for communication. Regular users are not shown here.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
