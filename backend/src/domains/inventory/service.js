...
const inventorySchema = z.object({
  ...
  quantity: z.union([z.number().min(0, 'Quantity must be non-negative'), z.string().transform(val => Number(val))]),
  unit: z.string().min(1, 'Unit is required'),
  cost_price: z.union([z.number().min(0, 'Cost price must be non-negative'), z.string().transform(val => Number(val))]),
  selling_price: z.union([z.number().min(0, 'Selling price must be non-negative'), z.string().transform(val => Number(val))]),
  reorder_level: z.union([z.number().min(0, 'Reorder level must be non-negative'), z.string().transform(val => Number(val))]).optional(),
